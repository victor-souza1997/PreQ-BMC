...
def export_4d(spec: EquivalenceSpec, lb: str, ub: str):
    _, channels, height, width = [str(v) for v in spec.input_shape]
...
{
    for(size_t i = 0; i < BATCH * CHANNELS * HEIGHT * WIDTH; i++)
    {
        input[i] = nondet_float();            
    }
}

static inline void add_input_assumptions(float input[BATCH * CHANNELS * HEIGHT * WIDTH], float x[BATCH][CHANNELS][HEIGHT][WIDTH])
{
    size_t i = 0;
    for(size_t b = 0; b < BATCH; b++)
    {
        for(size_t c = 0; c < CHANNELS; c++)
        {
            for(size_t h = 0; h < HEIGHT; h++)
            {
                ...
                {
                    x[b][c][h][w] = input[i];
                    __ESBMC_assume(lower_bounds[b][c][h][w] <= x[b][c][h][w] && x[b][c][h][w] <= upper_bounds[b][c][h][w]);
                    i++;   
                }
            }
        }
...
int main()
{
    ...
    float x[BATCH][CHANNELS][HEIGHT][WIDTH];
